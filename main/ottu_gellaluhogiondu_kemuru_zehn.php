<?php 
	include("conn.php");
	$getperiod_Query=mysqli_query($conn,"select * from `gelluonduhogu_kemuru_zehn` order by kramasankhye desc limit 1");
	...
	$chkResult=mysqli_query($conn,"select * from `gellaluhogiondu_kemeru_phalitansa_zehn` where `kalaparichaya`='".$periodid."'");
	$chkResultRow=mysqli_num_rows($chkResult);	
	if($chkResultRow == null){
		$selectData=mysqli_query($conn,"select * from `bajikattuttate_kemuru_zehn` where `kalaparichaya`='".$periodid."'");
		$selectdataRow=mysqli_num_rows($selectData);
		if($selectdataRow != null){
			$chksapregreenQuery=mysqli_query($conn,"select * from `bajikattuttate_kemuru_zehn` where `kalaparichaya`='".$periodid."'");
			...
			if($chksapregreenRow != null){
				$totalgreentabquery=mysqli_query($conn,"SELECT (SUM(ketebida)-(SUM(ketebida)/100*2)) as totalamount
					FROM
					`bajikattuttate_kemuru_zehn` where `kalaparichaya`='$periodid'");
