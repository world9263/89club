...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if (empty($err)) {
        ...
        mysqli_query($conn, "UPDATE hastacalita_phalitansa_funf SET sthiti='0'");

        $sql = "UPDATE hastacalita_phalitansa_funf SET sthiti='1' WHERE sankhye=$username";
        if ($conn->query($sql) === TRUE) {
            // Delete all rows from gelluonduhogu_zehn_zehn before inserting (optional, if needed)
            mysqli_query($conn, "DELETE FROM gelluonduhogu_zehn_zehn_3");

            // Insert the new value into gelluonduhogu_zehn_zehn
            $insertQuery = "INSERT INTO gelluonduhogu_zehn_zehn_3 (id, manual_number) VALUES (1, $username)
                            ON DUPLICATE KEY UPDATE manual_number = VALUES(manual_number)";

            if (mysqli_query($conn, $insertQuery)) {
                header("Location: wingo5min.php");
                exit();
            } else {
