...
<?php
    ...
    // if everything is ok, try to upload file
        ...
        if (move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)) {
            ...
            $usdt_images = $firebase->get('system_settings/usdt_images') ?: [];
            $newId = time() . mt_rand(10,99);
            $usdt_images[$newId] = ['filename' => $filename, 'status' => 0];
            $chk = $firebase->set('system_settings/usdt_images', $usdt_images);
  ...
  if(isset($_POST['upiid'])){
		$a_id = $_POST['upiid'];
		$usdt_images = $firebase->get('system_settings/usdt_images') ?: [];
		foreach ($usdt_images as $k => $u) {
		    if ($u['filename'] == $a_id) {
		        $usdt_images[$k]['status'] = 1;
		    } else {
		        $usdt_images[$k]['status'] = 0;
		    }
		}
		$firebase->set('system_settings/usdt_images', $usdt_images);
	}
      ...
      <div class="main-panel">
        <div class="content-wrapper">
		  ...
		  <div class="row">
			...
			<?php	foreach ($usdt_images as $row) {
			?>
